...
import { Loader2, AlertCircle, CheckCircle, Eye, EyeOff, Lock, ShieldCheck, Hash } from 'lucide-react';
...
export default function LoginStudent() {
    const { loginStudent, confirmLoginOtp } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const result = await loginStudent(form.student_id, form.password);
                ...
                <div style={s.glassCard} className="fade-in-up login-card">
                    ...
                    <form onSubmit={handleSubmit} style={s.form}>
                        <div style={s.fieldWrap}>
                            <label style={s.label} htmlFor="student-id">Student ID Number</label>
                            <div style={{ ...s.inputWrap, ...(focused.id ? s.inputWrapFocused : {}) }}>
                                <Hash size={18} color={focused.id ? '#fff' : 'rgba(255,255,255,0.5)'} style={s.fieldIcon} />
                                <input
                                    id="student-id"
                                    type="text"
                        ...
                        <div style={s.fieldWrap}>
                            <label style={s.label} htmlFor="student-pw">Password</label>
...
const globalCss = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .hero-text { display: flex; flex-direction: column; gap: 1.25rem; max-width: 580px; text-align: right; align-items: flex-end; animation-delay: 0.15s; }
    .login-card { animation-delay: 0s; }
    @media (max-width: 992px) {
        .login-wrapper { justify-content: center !important; padding: 1.25rem !important; }
        .hero-text { display: none !important; }
    }
    #student-id, #student-pw { background:transparent; border:none; outline:none; flex:1; color:#fff; font-family:'Outfit',sans-serif; font-size:1rem; min-width:0; }
    #student-id::placeholder, #student-pw::placeholder { color: rgba(255,255,255,0.4); }
...
const s = {
    ...
    bgOverlay: { position:'absolute', inset:0, background:'linear-gradient(200deg, rgba(8,12,28,0.85) 0%, rgba(15,22,45,0.65) 50%, rgba(249,115,22,0.2) 100%)' },
