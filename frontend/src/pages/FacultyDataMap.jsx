...
import { getFaculties, createFaculty, updateFaculty, deleteFaculty, getEligibilityCriteria, facultyCreateReport, sendOtp, verifyOtp } from '../api';
import { Users, Search, Printer, Building, Plus, Pencil, Trash2, X, Check, FileText, GraduationCap, AlertCircle, ShieldCheck, Mail } from 'lucide-react';
...
export default function FacultyDataMap() {
    ...
    const [pendingForm, setPendingForm] = useState(null); // form data waiting for OTP confirm
    ...
    // Step 1: validate form then send OTP to faculty email
    const handleSave = async (e) => {
        ...
        // Add mode — send OTP first
        setOtpSending(true);
        setOtpError('');
        try {
            await sendOtp('add_faculty', form.email);
            setPendingForm(form);
            setOtpStep(true);
            setOtpValue('');
        } catch (err) {
    ...
    // Step 2: verify OTP then actually create faculty
    const handleOtpConfirm = async (e) => {
        e.preventDefault();
        setSaving(true);
        setOtpError('');
        try {
            await verifyOtp(otpValue, 'add_faculty', pendingForm.email);
            await createFaculty(pendingForm);
            setOtpStep(false);
            setModal(null);
            setPendingForm(null);
            loadData();
    ...
    return (
        <div>
            ...
            {modal && (
                <Modal title={modal === 'add' ? 'Add New Faculty' : 'Edit Faculty'} onClose={() => { setModal(null); setOtpStep(false); setOtpError(''); }}>
                        ...
                        <form onSubmit={handleOtpConfirm} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 4 }}>
                                ...
                                <button type="submit" className="btn btn-primary" disabled={saving || otpValue.length !== 6}>
                                    <Check size={14} /> {saving ? 'Verifying...' : 'Confirm & Add Faculty'}
                                </button>
                            </div>
                        </form>
                    ) : (
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 8 }}>
                            ...
                            <button type="submit" className="btn btn-primary" disabled={saving || otpSending}>
                                {modal === 'add'
                                    ...
                                    : <><Check size={14} /> {saving ? 'Saving...' : 'Save Faculty'}</>
                                }
                            </button>
                        </div>
                    </form>
