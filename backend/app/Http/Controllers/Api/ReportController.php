...
class ReportController extends Controller
{
    ...
    {
        try {
            $summary = \App\Services\CacheService::getDashboardSummary();
    ...
    {
        try {
            $presets = \App\Services\CacheService::getReportPresets();
    ...
    public function search(Request $request): JsonResponse
    {
        try {
            ...
            $events = collect([]);
            try {
                $events = Event::select(['id', 'title', 'venue', 'date_start', 'status'])
                    ->where(function($query) use ($q) {
            ...
            \Log::error('Search endpoint error: ' . $e->getMessage(), [
                'query' => $request->get('q', ''),
